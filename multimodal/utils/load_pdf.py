...
class PDFSearch:
    def __init__(self):
        ...
        self.text = self.load_pdf() #  load the pdf and extract the text from it
        self.chunks = self.chunk_text() #split the text into chunks of 1500 characters to be able to calculate the embeddings for each chunk
        self.embeddings = self.calc_embeddings() # calculate the embeddings for the chunks of text
        self.index = self.faiss_index() # create a faiss index for the embeddings to be able to search for the closest text extracts
    ...
    def calc_embeddings(self) -> np.ndarray:
        ...
        """
        embeddings = self.model.encode(self.chunks) #calculate the embeddings for the chunks of text using the sentence transformer model
        return embeddings
